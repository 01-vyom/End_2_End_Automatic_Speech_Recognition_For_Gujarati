...
##4 Gram Word-Level Langauage Model
...
def prefix_beam_search(ctc, lm=None, lmchar = None, k=25, alphac=0.8, alphaw=0.3, beta=5, prune=0.001):
	"""
	...
	W = lambda l: re.findall(r'[\u0A81\u0A82\u0A83\u0A85\u0A86\u0A87\u0A88\u0A89\u0A8A\u0A8B\u0A8C\u0A8D\u0A8F\u0A90\u0A91\u0A93\u0A94\u0A95\u0A96\u0A97\u0A98\u0A99\u0A9A\u0A9B\u0A9C\u0A9D\u0A9E\u0A9F\u0AA0\u0AA1\u0AA2\u0AA3\u0AA4\u0AA5\u0AA6\u0AA7\u0AA8\u0AAA\u0AAB\u0AAC\u0AAD\u0AAE\u0AAF\u0AB0\u0AB2\u0AB3\u0AB5\u0AB6\u0AB7\u0AB8\u0AB9\u0ABC\u0ABD\u0ABE\u0ABF\u0AC0\u0AC1\u0AC2\u0AC3\u0AC4\u0AC5\u0AC7\u0AC8\u0AC9\u0ACB\u0ACC\u0ACD\u0AD0\u0AE0\u0AE1\u0AE2\u0AE3\u0AF1]+[\s|>]', l)
	F = ctc.shape[1]
			...
			for c in pruned_alphabet:
				...
				# END: STEP 4
					...
					# STEP 5: Extending with any other non-blank character and LM constraints
						...
						Pnb[t][l_plus] += lm_prob * ctc[t][c_ix] * (Pb[t - 1][l] + Pnb[t - 1][l]) * lmchar_prob
					else:
						Pnb[t][l_plus] += ctc[t][c_ix] * (Pb[t - 1][l] + Pnb[t - 1][l]) * lmchar_prob
					# END: STEP 5

					# STEP 6: Make use of discarded prefixes
					if l_plus not in A_prev:
						Pb[t][l_plus] += ctc[t][-1] * (Pb[t - 1][l_plus] + Pnb[t - 1][l_plus]) * lmchar_prob
						Pnb[t][l_plus] += ctc[t][c_ix] * Pnb[t - 1][l_plus] * lmchar_prob
					# END: STEP 6
...
def init(alphabet):
    ...
    temp_ls4 = []
    for i in range(le-3):
        temp_ls4.append(cor_ls[i]+" "+cor_ls[i+1]+" "+cor_ls[i+2]+" "+cor_ls[i+3])
